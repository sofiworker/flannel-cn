...
type NetConf struct {
	...
	// 默认值：/run/flannel/subnet.env
	SubnetFile    string                 `json:"subnetFile"`
	// 默认值：/var/lib/cni/flannel
...
func loadFlannelSubnetEnv(fn string) (*subnetEnv, error) {
	...
	// 宿主机上 /run/flannel/subnet.env 文件
	// FLANNEL_NETWORK=172.18.0.0/16   这里是 pod 的 cidr
...
// 这里将 /etc/cni/net.d/10-flannel.conflist 配置信息获取到
